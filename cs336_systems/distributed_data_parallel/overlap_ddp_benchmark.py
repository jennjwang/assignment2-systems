...
def profile_overlap_ddp(trace_name: str = "ddp_overlap", warmup_steps: int = 1, measurement_steps: int = 3):
    import subprocess
    import sys
...
def run_profile_target(warmup_steps: int, measurement_steps: int):
    ...
    if num_gpu < 1:
        raise RuntimeError("No CUDA devices available for overlap DDP profiling")
...
def main():
    ...
    with open("overlap_ddp_benchmark.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=result.keys())
        writer.writeheader()
        writer.writerow(result)
    for k, v in result.items():
        print(f"{k}: {v}")
    print(profile_overlap_ddp.remote("ddp_overlap"))
